...
const contactFormSchema = z.object({
  ...
  phone: z.string().max(20, "Phone number can have at most 20 characters").optional(),
...
export default function InlineContactFormEn({ className }: InlineContactFormProps) {
  ...
  const submitContactMutation = useMutation({
    ...
    onError: (error: unknown) => {
      setSubmissionStatus({
        type: "error",
        message: error instanceof Error ? error.message : "Failed to send the form. Please try again later.",
      })
    },
  })
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-3">
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel className="text-sm text-white">Phone</FormLabel>
                <FormControl>
                  <Input
                    id="phone"
                    type="tel"
