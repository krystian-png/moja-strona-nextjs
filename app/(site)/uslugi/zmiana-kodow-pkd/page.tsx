...
const faqItems = [
  ...
  { question: 'Co stanie się z kodami PKD po 31 grudnia 2026 r.?', answer: 'Zależy od tego, jaki kod figuruje w dziale 3. Jeżeli odpowiada mu dokładnie jeden kod PKD 2025, system podmienia go jeden do jednego. Jeżeli zakres kodu rozdzielono między kilka kodów, system wpisuje ten wskazany w kluczu interpretacyjnym. Jeżeli nie odpowiada mu żaden, kod zostaje wykreślony bez wpisania nowego. Odrębna sytuacja dotyczy kodów wpisanych na poziomie działu albo grupy — tam przy 112 kodach klucze nie wskazują, który odpowiednik wybrać.' },
  ...
  { question: 'Jak sprawdzić, jaki kod PKD 2025 odpowiada mojemu staremu kodowi?', answer: 'Odpowiedniki wynikają z kluczy przejścia opublikowanych przy rozporządzeniu PKD 2025. Możesz je sprawdzić w wyszukiwarce na tej stronie — obsługuje kody na każdym poziomie, od samego działu (43) przez grupę (43.1) i klasę (43.11) po pełny kod (43.11.Z). Pokazuje wszystkie odpowiedniki oraz ten, który wpisze system, jeżeli klucze go wskazują.' },
  { question: 'Co to jest klucz interpretacyjny PKD?', answer: 'Gdy zakres jednego kodu PKD 2007 rozdzielono między kilka kodów PKD 2025, w kluczach przejścia jeden z nich bywa wskazany jako uproszczona relacja jeden do jednego. To właśnie klucz interpretacyjny. Powstał po to, żeby automatyczna konwersja mogła się odbyć bez udziału człowieka. Nie jest oceną, który kod jest właściwy dla konkretnej spółki. Występuje wyłącznie przy kodach na poziomie klasy i podklasy — przy kodach na poziomie działu i grupy klucze takiego wskazania nie zawierają.' },
  { question: 'Czy jeden kod PKD 2007 może odpowiadać kilku kodom PKD 2025?', answer: 'Tak. Spośród 1631 kodów ujętych w kluczach przejścia 455 ma więcej niż jeden odpowiednik. Rekordzista to 47.91.Z, którego zakres rozdzielono między 37 podklas PKD 2025. Przy 343 z tych kodów klucze wskazują, który z odpowiedników wpisać. Przy pozostałych 112 — nie wskazują.' },
  { question: 'Czy system może wpisać niewłaściwy kod PKD?', answer: 'Może wpisać kod, który nie odpowiada faktycznej działalności spółki. Nie jest to błąd — przy kodach na poziomie klasy i podklasy system działa dokładnie zgodnie z kluczem interpretacyjnym. Rzecz w tym, że klucz przygotowano na potrzeby masowej konwersji, a nie oceny pojedynczej spółki, a sąd rejestrowy przy automatycznej wymianie nie bada, czym spółka się zajmuje. Przy kodach na poziomie działu i grupy klucze w ogóle nie zawierają wskazania, więc nie wiadomo, jaki kod system wpisze i czy wpisze jakikolwiek.' },
        ...
        <section className="mx-auto max-w-6xl space-y-10">
          <div id="oferta" className={`${card} scroll-mt-28`}>
            ...
            <div className="space-y-4 text-lg text-white/90">
              ...
              <div><strong>2. Dobór kodów PKD 2025</strong><p>Na podstawie informacji o działalności, które nam przekażesz, proponujemy podklasy PKD 2025 odpowiadające temu, czym spółka faktycznie się zajmuje. Nie ograniczamy się do przepisania klucza interpretacyjnego.</p></div>
          ...
          <div className={card}>
            ...
            <div className="space-y-5 text-lg text-white/90">
              ...
              <div><strong>W rejestrze mieści się dziesięć pozycji, w tym jedna przeważająca.</strong><p>Wynika to z art. 40 pkt 1 ustawy o KRS. Umowa spółki może wymieniać znacznie więcej rodzajów działalności — w rejestrze ujawnia się tylko wybrane. Trzeba więc zdecydować nie tylko, czym zastąpić stare kody, ale też które działalności w ogóle ujawnić.</p></div>
            </div>
          </div>

          <div className="grid gap-6 md:grid-cols-2">
            <div className={card}><h2 className="mb-3 text-2xl font-bold text-amber-400">Do 31 grudnia 2026 r. wybierasz Ty</h2><p className="text-lg text-white/90">Po zakończeniu okresu przejściowego kody w dziale 3 wymienia system, na podstawie kluczy przejścia (art. 20e ust. 1 ustawy o KRS). Wcześniej decyzja należy do spółki.</p></div>
          ...
          <div className={card}>
            ...
            <div className="space-y-4 text-lg text-white/90">
              <p><strong>Powiązanie jednoznaczne.</strong> Kodowi PKD 2007 odpowiada dokładnie jeden kod PKD 2025. System podmienia kod jeden do jednego, wpis pozostaje prawidłowy. Dotyczy 1176 z 1631 kodów ujętych w kluczach przejścia.</p>
              <p><strong>Powiązanie wieloznaczne.</strong> Zakres kodu został w nowej klasyfikacji rozdzielony między kilka kodów. System wpisuje ten, który wskazano w kluczu interpretacyjnym. Nikt nie bada, którą z tych działalności spółka faktycznie prowadzi. Dotyczy 455 kodów; skrajny przypadek to 47.91.Z, którego zakres rozdzielono między 37 podklas.</p>
              <p><strong>Brak dopasowania.</strong> Jeżeli kodowi nie odpowiada żaden kod PKD 2025, system wykreśla go bez wpisania nowego.</p>
              <p>Automatyczna wymiana nie obejmuje działalności pożytku publicznego (art. 20e ust. 2 ustawy o KRS).</p>
              <p>Osobna sytuacja dotyczy kodów wpisanych do rejestru na poziomie działu albo grupy, a więc krótszych niż pełny pięcioznakowy kod. Przy 112 takich kodach klucze przejścia wskazują kilka odpowiedników i nie określają, który z nich należy wpisać. Który z trzech powyższych wariantów znajdzie do nich zastosowanie, nie wynika ani z przepisów, ani z dokumentów opublikowanych przez Główny Urząd Statystyczny.</p>
            </div>
          ...
          <div className={card}>
            ...
            <p className="text-lg text-white/90">288 kodów PKD 2007 nie ma swojego numeru w PKD 2025. Nie znaczy to, że znikną – każdemu klucz przejścia przypisuje jakiś odpowiednik. Znaczy to co innego: <strong>jeżeli którykolwiek z nich figuruje dziś w dziale 3, wpis spółki na pewno nie był aktualizowany od wejścia w życie PKD 2025.</strong></p>
